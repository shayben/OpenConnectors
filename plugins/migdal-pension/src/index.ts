/**
 * Migdal Insurance Pension Plugin — MCP Server Entry Point
 *
 * Exposes two MCP tools for Migdal Insurance (מגדל ביטוח):
 *
...
server.tool(
  ...
  "Fetch Migdal pension fund balance, contributions, and investment returns.",
  {
    year: z
      .number()
      .optional()
    ...
    if (!nationalId || !password) {
      return {
        content: [
          {
            type: "text" as const,
            text: "Error: Missing credentials. Run: openconnectors vault set migdal-pension national_id && openconnectors vault set migdal-pension password",
          },
        ],
        isError: true,
      };
...
server.tool(
  "fetch_tax_documents",
  "Fetch Migdal annual pension fund statement for a given tax year.",
  {
...
main().catch((err) => {
  console.error("Fatal error starting migdal-pension MCP server:", err);
  process.exit(1);
});
