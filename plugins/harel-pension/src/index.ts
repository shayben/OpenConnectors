/**
 * Harel Insurance & Finance Pension Plugin — MCP Server Entry Point
 *
 * Exposes two MCP tools for Harel (הראל ביטוח ופיננסים):
 *
...
server.tool(
  ...
  "Fetch Harel pension fund balance, contributions, and investment returns.",
  {
    year: z
      .number()
      .optional()
    ...
    if (!nationalId || !password) {
      return {
        content: [
          {
            type: "text" as const,
            text: "Error: Missing credentials. Run: openconnectors vault set harel-pension national_id && openconnectors vault set harel-pension password",
          },
        ],
        isError: true,
      };
...
server.tool(
  "fetch_tax_documents",
  "Fetch Harel annual pension fund statement for a given tax year.",
  {
...
main().catch((err) => {
  console.error("Fatal error starting harel-pension MCP server:", err);
  process.exit(1);
});
